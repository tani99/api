...
from tables.util import VectorSpaceClusterer
...
class KMeansClusterer(VectorSpaceClusterer):
    ...
    def cluster_vectorspace(self, vectors, texts, trace=False):
        ...
        for trial in range(self._repeats):
            ...
            self._cluster_vectorspace(vectors, texts, trace)
    ...
    def _cluster_vectorspace(self, vectors, texts, trace=False):
        if self._num_means < len(vectors):
            ...
            while not converged:
                ...
                clusters_texts = [[] for m in range(self._num_means)]
                for i, vector in enumerate(vectors):
                    text = texts[i]
                    index = self.classify_vectorspace(vector)
                    # if len(clusters[index]) == 0:
                    #     clusters[index].append(vector)
                    #     clusters_texts[index].append(text)
                    #     # print("empty")
                    # elif len(clusters[index]) > 0:
                    #
                    #     print("test")
                    #     print(texts[i])
                    #     print(clusters_texts[index])
                    #     print(self.is_neighbouring_cluster(texts[i], clusters_texts[index]))
                    #     if self.is_neighbouring_cluster(texts[i], clusters_texts[index]):
                    #         # if self.distance_to_closest_cluster(texts[i], clusters_texts[index]) == 1:
                    #         print("consecutive")
                    #         clusters[index].append(vector)
                    #         clusters_texts[index].append(text)
                    #     else:
                    ...
                    #
                    # else:
                    #     print("Didn't satisfy any condition")
                ...
                if difference < self._max_difference:
                    ...
                    print("CLUSTERS: ")
                    print(clusters)
                    print(clusters_texts)

                print("means", new_means)
                # remember the new means
                self._means = new_means

    def is_neighbouring_cluster(self, text, cluster):
        for sentence in cluster:
            if not text in self._matrix[sentence]:
                return False
        return True

    def distance_to_closest_cluster(self, text, cluster):

        min_dist = self._matrix[text][cluster[0]]
        for sentence in cluster:
            if self._matrix[text][sentence] < min_dist:
                min_dist = self._matrix[text][sentence]
        return min_dist
