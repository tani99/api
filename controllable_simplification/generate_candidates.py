...
def main(args):
    ...
    os.system("cp " + args.input + " controllable_simplification/DiscourseSimplification/input.txt")
    os.chdir('controllable_simplification/DiscourseSimplification')
    os.system("mvn clean compile exec:java")
    os.chdir(cwd)
    dissim_candidates = dissim.generate_candidates(args.input, "controllable_simplification/DiscourseSimplification/output_dt.txt")
