...
from .edmundson_length import EdmundsonLengthMethod
from final_files.summarisation.nlp.stemmers import null_stemmer
...
class EdmundsonSummarizer(AbstractSummarizer):
    ...
    def __init__(self, stemmer=null_stemmer, cue_weight=1.0, key_weight=1.0,
                 title_weight=1.0, location_weight=0.0, length_weight=1.0):
        ...
        self._length_weight = float(length_weight)
    ...
    def summarise(self, document, sentences_count):
        ...
        if self._cue_weight > 0.0:
            ...
            new_ratings = method.rate_sentences(document)
        ...
        #     ratings = self._update_ratings(ratings, method.rate_sentences(document))
        if self._length_weight > 0.0:
            method = self._build_length_method_instance()
            new_ratings = method.rate_sentences(document)
            ratings = self._update_ratings(ratings, method.rate_sentences(document))
    ...
    def length_method(self, document, sentences_count):
        summarization_method = self._build_location_method_instance()
        return summarization_method(document, sentences_count)

    def _build_length_method_instance(self):
        # print("LENGTH")
        self.__check_null_words()

        return EdmundsonLengthMethod(self._stemmer, self._null_words)
